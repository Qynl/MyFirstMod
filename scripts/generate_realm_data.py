...
# Broad basins support traversable lowlands; folded ridges and shallow ravines add relief.
ridges = add(.55,mul(-1.1,{'type':'minecraft:abs','argument':noise('strata',1,.12)}))
terrain = add({'type':'minecraft:y_clamped_gradient','from_y':8,'to_y':168,'from_value':1.8,'to_value':-1.8},
              add(mul(.8,noise('basins',1,0)),add(ridges,add(mul(.35,noise('climate',1,0)),mul(.23,noise('fractures',1,.5))))))
...
for name,temp,fog,sky,particle in [('hushed_grove',[-1,-.3],0x102F38,0x132535,'sculk_soul'),('luminous_fen',[-.3,.15],0x183F42,0x193E47,'spore_blossom_air'),('prism_wastes',[.15,.55],0x302646,0x271839,'end_rod'),('cinder_steps',[.55,1],0x352B38,0x281A30,'ash')]:
    ...
    put('worldgen/biome/'+name+'.json',{'has_precipitation':False,'temperature':.4,'downfall':0,'effects':{'fog_color':fog,'sky_color':sky,'water_color':0x31667F,'water_fog_color':0x122333,'particle':{'options':{'type':'minecraft:'+particle},'probability':.004},'ambient_sound':'minecraft:ambient.'+('basalt_deltas.loop' if name=='cinder_steps' else 'warped_forest.loop' if name=='luminous_fen' else 'soul_sand_valley.loop'), 'mood_sound':{'sound':'minecraft:ambient.cave','tick_delay':8000,'block_search_extent':8,'offset':2}},'spawners':{'monster':[{'type':'myfirstmod:rift_sentinel','weight':25,'minCount':1,'maxCount':2},{'type':'myfirstmod:shardstalker','weight':35,'minCount':1,'maxCount':2}]},'spawn_costs':{},'carvers':{},'features':[[],[],[],[],['myfirstmod:realm_ruins','myfirstmod:waystone_shrine','myfirstmod:rift_observatory','myfirstmod:mourning_cathedral','myfirstmod:forgotten_memorial'],[],['myfirstmod:realm_resources'],[],[],['myfirstmod:realm_flora','myfirstmod:realm_scenery'],[]]})
