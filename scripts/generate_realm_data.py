...
def block(name): return {'type':'minecraft:block','result_state':{'Name':'minecraft:'+name}}
...
router.update(temperature=noise('climate',1,0),initial_density_without_jaggedness=terrain,final_density={"type":"minecraft:squeeze","argument":{"type":"minecraft:interpolated","argument":terrain}})
...
for biome, top in [('hushed_grove','sculk'),('prism_wastes','calcite'),('cinder_steps','smooth_basalt')]:
...
put('worldgen/noise_settings/null_realm.json', {'sea_level':0,'disable_mob_generation':False,'aquifers_enabled':False,'ore_veins_enabled':False,'legacy_random_source':False,'default_block':{'Name':'minecraft:deepslate'},'default_fluid':{'Name':'minecraft:air'},'noise':{'min_y':-64,'height':384,'size_horizontal':1,'size_vertical':2},'noise_router':router,'surface_rule':{'type':'minecraft:sequence','sequence':rules},'spawn_target':[]})
biomes = []
for name,temp,fog,sky,particle in [('hushed_grove',[-1,-.2],0x102F38,0x132535,'sculk_soul'),('prism_wastes',[-.2,.35],0x302646,0x271839,'end_rod'),('cinder_steps',[.35,1],0x352B38,0x281A30,'ash')]:
