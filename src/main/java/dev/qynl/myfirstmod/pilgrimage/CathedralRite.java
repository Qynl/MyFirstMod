...
public final class CathedralRite {
    ...
    public static ActionResult interact(ServerPlayerEntity player,BlockPos pos) {
        ...
        for(int i=0;i<SEALS.length;i++) {
            ...
            world.playSound(null,pos,next==0?SoundEvents.BLOCK_RESPAWN_ANCHOR_DEPLETE:SoundEvents.BLOCK_AMETHYST_BLOCK_RESONATE,SoundCategory.BLOCKS,1,next==0?.5f:.7f+next*.2f);
