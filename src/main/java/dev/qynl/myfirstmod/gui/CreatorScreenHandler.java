...
public class CreatorScreenHandler extends ScreenHandler {
    ...
    @Override public boolean onButtonClick(PlayerEntity player,int id){ if(!(player instanceof ServerPlayerEntity sp))return true; UnitWorldData d=UnitWorldData.get(sp.getServer());
        if(id==0){UnitDefinition u=new UnitDefinition("guard","Village Guard",Identifier.of("minecraft","villager"));u.description="A configurable faction defender.";u.factionId="village";u.maxHealth=30;u.attackDamage=7;u.equipment.put(EquipmentSlot.HEAD,new ItemStack(Items.IRON_HELMET));u.equipment.put(EquipmentSlot.CHEST,new ItemStack(Items.IRON_CHESTPLATE));u.equipment.put(EquipmentSlot.LEGS,new ItemStack(Items.IRON_LEGGINGS));u.equipment.put(EquipmentSlot.FEET,new ItemStack(Items.IRON_BOOTS));u.equipment.put(EquipmentSlot.MAINHAND,new ItemStack(Items.IRON_SWORD));u.equipment.put(EquipmentSlot.OFFHAND,new ItemStack(Items.SHIELD));d.saveUnit(u);}
